...
int main(){
		...
		if (my_rank == 0){
			...
			/*MPI_Ssend(&rank0, 1, MPI_INT, 1, 17, MPI_COMM_WORLD);*/ /*standard send*/
			...
			MPI_Ssend(&rank1, 1, MPI_INT, 0, 23, MPI_COMM_WORLD); /*synchronous send*/
			/*MPI_Send(&rank1, 1, MPI_INT, 0, 23, MPI_COMM_WORLD);*/  /*standard send*/
		}
		...
		for(i = 0; i < 50; ++i){
			if (my_rank == 0){
				MPI_Ssend(&rank0, 1, MPI_INT, 1, 17, MPI_COMM_WORLD);
				/*MPI_Send(&rank0, 1, MPI_INT, 1, 17, MPI_COMM_WORLD);*/ /*standard send*/
				MPI_Recv(&rank1, 1, MPI_INT, 1, 23, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
