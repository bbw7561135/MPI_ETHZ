...
/*MatCreate(PETSC_COMM_WORLD,PETSC_DECIDE,PETSC_DECIDE,m*n,m*n,&A);   2.2.1 */
  MatSetFromOptions(A); 
  MatMPIAIJSetPreallocation(A,5,PETSC_NULL,5,PETSC_NULL); /* 3.3.0 */
  MatSeqAIJSetPreallocation(A,5,PETSC_NULL); /* 3.3.0 */
  MatSetUp(A); /* 3.3.0 */
  /* 
...
exit;
  ...
  */
  ...
  if ( !flg) { 
    ...
    for (II=Istart; II<Iend; II++) {
      ...
      if (v != 0) ; VecSetValues(b,1,&II,&v,INSERT_VALUES);
      v = /*u(i, j):*/ h * (i+1); VecSetValues(u,1,&II,&v,INSERT_VALUES);
    }
    VecAssemblyBegin(b); VecAssemblyBegin(u);
    VecAssemblyEnd(b); VecAssemblyEnd(u);
  } else { 
    /* PetscRandomCreate(PETSC_COMM_WORLD,&rctx); */
    VecSetRandom(u,/*rctx*/ PETSC_NULL); /* PetscRandomDestroy(&rctx); */
    MatMult(A,u,b); 
  } 
  ...
  */
  ...
  if (flg) { 
    ...
    for (II=Istart; II<Iend; II++) {
      i = II/n;  j = II - i*n;
      PetscSynchronizedPrintf(PETSC_COMM_WORLD, "%8.6f ", xx[II-Istart]); 
  ...
  if (flg) { /* view solution grid in an X window */
    PetscScalar *xx;
    DM  da; /* in PETSC 3.1 and earlier, DM and DMDA was DA */
    AO  ao;
    Vec x_da; 
    ...
    DMCreateGlobalVector(da,&x_da);
    DMDAGetAO(da,&ao);
    VecGetOwnershipRange(x, &Istart, &Iend);
    VecGetArray(x,&xx);
    for (II=Istart; II<Iend; II++) {
      i = II; AOApplicationToPetsc(ao,1,&i);
      VecSetValues(x_da, 1, &i, &xx[II-Istart], INSERT_VALUES);
    }
    VecRestoreArray(x, &xx);
